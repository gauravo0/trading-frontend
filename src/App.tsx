...
function App() {
  ...
  const fetchComparison = async () => {
    try {
      ...
      const res = await fetch("http://localhost:8080/api/stocks/compare", {
        method: "POST",
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#0f172a", color: "white" }}>
      ...
      <div style={{ flex: 1, padding: "20px" }}>
        ...
        <div style={{ marginTop: "20px", display: "flex", gap: "10px" }}>
          <input
            placeholder="Enter stock (RELIANCE.NS)"
            value={symbol}
            onChange={(e) => setSymbol(e.target.value)}
            style={{ padding: "10px", borderRadius: "8px", border: "none" }}
          />
          <button onClick={fetchStock} style={{ padding: "10px 20px", background: "#22c55e", borderRadius: "8px" }}>
            Search
          </button>
        </div>
        ...
        <div style={{ marginTop: "30px" }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            />
            <button onClick={fetchComparison} style={{ padding: "10px 20px", background: "#3b82f6", borderRadius: "8px" }}>
              Compare
            </button>
          </div>
